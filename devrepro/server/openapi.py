...
API_ROUTES: tuple[RouteSpec, ...] = (
    ...
    (
        "POST",
        f"{_API_PREFIX}/projects/{{project_id}}/members",
        "addProjectMember",
        "Give one person a role on one project, so baselines are team-scoped",
        True,
    ),
    (
        "GET",
        f"{_API_PREFIX}/projects/{{project_id}}/members",
        "listProjectMembers",
        "Who may administer this project's baseline",
        True,
    ),
    (
        "GET",
        f"{_API_PREFIX}/fleet/onboarding",
        "fleetOnboarding",
        "Aggregate time from enrolment to first READY. No machine or user is returned",
        True,
    ),
    (
        "POST",
        f"{_API_PREFIX}/fleet/simulate-policy",
        "simulatePolicy",
        "Who would this policy break, evaluated against stored snapshots",
        True,
    ),
