...
def test_tool_versions_styles():
    ...
    # `.tool-versions` now carries the same "Generated by" header every other
    # file this project emits carries, so the assertion is on the entry rather
    # than on the whole document. asdf treats `#` as a comment.
    entries = [ln for ln in asdf.splitlines() if ln and not ln.startswith("#")]
    assert entries == ["node 20.0.0"]
    assert "Generated by devrepro-doctor" in asdf
